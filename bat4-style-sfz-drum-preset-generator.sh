...
# key 50-56
  "$d2" "$d12" "$d4" "$d13" "$d6" "$d14" "$d6"
# key 57-63
  "$d8" "$d9" "$d10" "$d1" "$d1" "$d2" "$d3"
# key 64-70
  "$d4" "$d13" "$d6" "$d14" "$d6" "$d8" "$d9"
# key 71-77
  "$d10" "$d1" "$d11" "$d2" "$d3" "$d15" "$d5"
# key 78-84
...
SFZ_FILENAME="CHAT_NF2BAT4BBE49_copy0-${RANDOM_WORD}-$((RANDOM % 999 + 100)).sfz"
